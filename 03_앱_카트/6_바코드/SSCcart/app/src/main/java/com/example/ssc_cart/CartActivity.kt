...
class CartActivity : AppCompatActivity() {
    ...
    override fun onStart() {
        ...
        btn_godb.setOnClickListener {
            goDB()
            Toast.makeText(this,"계산이 완료되었습니다.", Toast.LENGTH_LONG).show()
        }
    }
                    ...
                    .get().addOnSuccessListener { documents ->
                        for (document in documents) {
                            if (document.data.toMap().get("price") != null){
                                ...
                                db.collection("Purchase").document(temp_date).update("total_price", total)
                                db.collection("Purchase").document(temp_date).update("each_product", FieldValue.arrayUnion(PRODUCT(document.data.toMap().get("name") as String?, b*((document.data.toMap().get("weight") as Long?)!!), 1, b*100)))
                            }
                        }
                    }
        }
