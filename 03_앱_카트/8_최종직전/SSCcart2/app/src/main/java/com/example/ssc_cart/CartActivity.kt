    ...
    fun ankoWeightAlertDialog(name: String?) {
     ...
     private fun addCartWeight(weight_result: Int, weight_name:String?) : Boolean {
         ...
         if (Purchase[weight_name] == null) {
                 ...
                 ?.times(weight_result?.toLong()))?.div(100)), all_product_db[weight_name]?.geturl()!!)}")
                 Purchase.put(weight_name.toString(),
                     ((all_product_db[weight_name]?.getweight()
                         ?.times(weight_result?.toLong()))?.div(100))!!)
                 ...
                 Log.d("엠큐티티 이후 첫 아이템", "${remove_data}")
                 items.remove(remove_data)
                 Purchase.set(weight_name.toString(), Purchase[weight_name]?.plus(all_product_db[weight_name]?.getweight()?.times(weight_result)?.div(100)!!) as Long)
                 items.add(CartData(weight_name, all_product_db[weight_name]?.getweight()!!, abc, Purchase[weight_name], all_product_db[weight_name]?.geturl()!!))
                 all_total = all_total?.plus((all_product_db[weight_name]?.getweight()?.times(weight_result?.toLong()))?.div(100)!!)
                 abc = weight_result.toLong()?.plus(abc)
         }
